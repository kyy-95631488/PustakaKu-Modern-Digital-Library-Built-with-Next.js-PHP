...
export default function LoginPage() {
  ...
  const promptVerification = async (email) => {
    try {
      showLoading();
      const sendResp = await axios.post("http://localhost/v1/auth/sendVerification/", { email });
      hideLoading();
      ...
      if (code) {
        try {
          showLoading();
          const resp = await axios.post("http://localhost/v1/auth/verifCode/", { email, code });
          hideLoading();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      showLoading();
      const { data } = await axios.post("http://localhost/v1/auth/login/", form);
      hideLoading();
